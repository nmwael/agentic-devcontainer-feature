...
- **GPU Architectures**: Repo bakes in 80, 86, 89, 90 (Ampere/Ada/Hopper). Turing (`75`) requires manual addition to `CMAKE_CUDA_ARCHITECTURES`.
...
## Operational Notes
- **Model Swapping**: Can be done without rebuilding the binary.
- **Context Exhaustion**: Symptoms include truncated responses or "context length exceeded" errors. Use `-lv` for verbose logging.
