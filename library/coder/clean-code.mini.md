...
## Testable Code
Prioritize writing code that is easy to unit test. This often means decomposing complex logic into small, single-responsibility functions.

Key principles for testability:
1.  **Single Responsibility:** A function should do one thing well. Small functions are easier to reason about and have fewer execution paths to test.
2.  **Avoid Side Effects:** Minimize hidden side effects (e.g., modifying global state, direct I/O, or mutating shared objects). Functions that take inputs and return outputs without side effects are "pure" and significantly easier to test reliably.
3.  **Dependency Injection:** Instead of hardcoding dependencies (like database connections or file system access) inside a function, pass them as arguments. This allows you to easily swap real components for mocks or stubs during testing.
4.  **Example - Hard to Test:** A function that reads a file, parses the content, and updates a global counter is difficult to test because it involves I/O and shared state.
5.  **Example - Easy to Test:** Separate these into three functions: one that takes a string and returns a parsed object; one that takes an object and returns a new count; and a main coordinator that handles the I/O and calls the others.
