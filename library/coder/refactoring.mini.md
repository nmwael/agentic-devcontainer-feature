...
Refactoring must only occur once the "Green" state (passing tests) is achieved. Never refactor code that is currently failing its tests or whose behavior is not yet verified.

To maintain test coverage during complex logic changes:
1.  **Characterize:** If behavior is unclear, write characterization tests to lock in current behavior before any structural change.
2.  **Small Steps:** Break down large refactorings into the smallest possible atomic transformations that preserve behavior and remain runnable at every step.
3.  **Verify Constantly:** Run the test suite after every small transformation. If a test fails, revert or fix the immediate change before proceeding.
4.  **Coverage Preservation:** Ensure that any new logic added during refactoring is immediately covered by new tests to prevent regression.
